...
public class SaldoService {
     ...
     * @throws SecurityException caso o usuário que está tentando atualizar a
     *                           receita não seja o dono da mesma
     * @author Otto Fidelis
     * @since 1.0
     * @version 1.0
     */
    public SaldoModel update(UsuarioModel usuario, ReceitasModel receitaNova, double quantiaAntiga) {
        ...
        SaldoModel saldo = findLastSaldo(usuario);
        BigDecimal quantia = BigDecimal.valueOf(saldo.getQuantia()).add(BigDecimal.valueOf(receitaNova.getQuantia()))
                .subtract(BigDecimal.valueOf(quantiaAntiga));
        saldo.setQuantia(quantia.doubleValue());
        return saldoRepository.save(saldo);
    }
     ...
     * @return saldo atualizado
     * @throws SecurityException caso o usuário que está tentando atualizar a
     *                           despesa não seja o dono da mesma
     * @author Otto Fidelis
     * @since 1.0
     * @version 1.0.1
     */
    public SaldoModel update(UsuarioModel usuario, DespesasModel despesaNova, double quantiaAntiga) {
        ...
        SaldoModel saldo = findLastSaldo(usuario);
        BigDecimal quantia = BigDecimal.valueOf(saldo.getQuantia())
                .subtract(BigDecimal.valueOf(despesaNova.getQuantia())).add(BigDecimal.valueOf(quantiaAntiga));
     ...
     * @return saldo atualizado
     * @throws SecurityException caso o usuário que está tentando atualizar o saldo
     *                           não seja o dono do mesmo
     * @author Otto Fidelis
     * @since 1.0
     * @version 1.0
     */
     ...
     * @throws SecurityException      se o usuário não tiver permissão para acessar
     *                                o saldo, ou seja, se o saldo não pertencer ao
     *                                usuário
     * @author Otto Fidelis
     * @since 1.0
     * @version 1.0
    ...
    /**
     * <p>
     * Remove um saldo do sistema pelo ID
     * </p>
     * <strong>CUIDADO: </strong>
     * <p>
     * Isso é uma ação permanente e não pode ser desfeita.
     * </p>
     * <p>
     * Ao deletar uma entidade, não se esqueça de verificar se ela não irá deixar
     * outras entidades órfãs
     * </p>
     * 
     * @param id      O ID do saldo a ser removido
     * @param usuario O usuário que está solicitando a remoção do saldo
     * @throws SaldoNotFoundException se não encontrar o saldo com o ID especificado
     * @throws SecurityException      se o usuário não tiver permissão para remover
     *                                o saldo, ou seja, se o saldo não pertencer ao
     *                                usuário
     * @author Otto Fidelis
     * @since 1.0
     ...
     * @throws SaldosNotFoundException se não encontrar nenhum saldo para o usuário
